...
const Shopall = () => {
  ...
  const handleAddToWishlist = (variant, e) => {
  ...
  const filteredProducts = ProductData.filter(
    (product) => product.variants?.length > 0 && product.isActive === true
  );
  ...
  if (filteredProducts.length === 0) {
    return <div className="text-center py-10">No active products available.</div>;
  }

  return (
    <section className="secNewLaunches container mx-auto bg-white mb-10">
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-2xl font-bold mb-6 capitalize">All Products</h2>
